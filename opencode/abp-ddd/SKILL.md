...
public class DistrictAppService : AbstractKeyCrudAppService<District, DistrictDto, DistrictKey>
{
    ...
    protected override Task DeleteByIdAsync(DistrictKey id) => Repository.DeleteAsync(d => d.CityId == id.CityId && d.Name == id.Name);
    protected override Task<District> GetEntityByIdAsync(DistrictKey id) => AsyncQueryableExecuter.FirstOrDefaultAsync((await Repository.GetQueryableAsync()).Where(d => d.CityId == id.CityId && d.Name == id.Name));
}
```
...
{
    public virtual async Task FooAsync() { }  // virtual olmalı
}
```
